...
int GetMWInfo(int fd)
{
	...
	int n = SerialRead(rec_buf, 6 + 7);  // read up to 100 characters if ready to read
...
int GetMotorMW(int fd, motor_data *md)
{
	...
	//read command
	unsigned char rec_buf [100];
	int n = SerialRead(rec_buf, 6 + 8*2);  // read up to 100 characters if ready to read
...
int GetSensorMW(int fd, sensor_data *sd)
{
	...
	//read command
	unsigned char rec_buf [100];
	int n = SerialRead(rec_buf, 6 + 9*2);  // read up to 100 characters if ready to read
...
int GetAltitudeMW(int fd, altitude_data *altd)
{
	...
	int n = SerialRead(rec_buf, 6 + rec_data_length);  // read up to 100 characters if ready to read
...
int SetRcMW(int fd, rc_values *values) 
{
	...
	checksum = 0;
	Serialize8(data_length);
	Serialize8(MSP_BEAGLEBOARD);
	Serialize16(values->roll);
	Serialize16(values->pitch);
	Serialize16(values->yaw);
	Serialize8(checksum);
	
	int bytes_write = SerialWrite(send_buf, data_length + 6);
	...
	//read command
	unsigned char rec_buf[100];
	int n = SerialRead(rec_buf, 100); // read up to 100 characters if ready to read
	//printf("SetRcMW: errno: %d\n", errno);
