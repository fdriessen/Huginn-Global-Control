...
int main( int argc, char* argv[] )
{
	...
	long number_of_frames = cvGetCaptureProperty(input_video,CV_CAP_PROP_FRAME_COUNT);

	if(number_of_frames>4294967296) //unlikely case
	{
		fprintf(stderr, "Error: Input file is too long, should be max 2^32 frames.\n");
		return -1;
		// can be solved by not using the track bar for the video position or by
		// normalizing the value, track bar can only use an int
	}
	...
	// track bar to set the number of features
	createTrackbar( " Number of features", "Optical Flow", &number_of_features, MAX_FEATURES);
	...
	while(true)
	{
		 ...
		 * "number_of_features" will be set to a value <= MAX_FEATURES indicating the number of feature points found.
		 */
		if(!number_of_features) number_of_features = 1; // should not be 0
		...
		// track bar to set the video position
		createTrackbar( " Video Position", "Optical Flow", &current_frame, (number_of_frames - 1));

		// don't get out of bound and loop video
